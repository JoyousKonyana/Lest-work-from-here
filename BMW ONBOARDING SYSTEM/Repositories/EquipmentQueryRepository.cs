...
namespace BMW_ONBOARDING_SYSTEM.Repositories
{
    public class EquipmentQueryRepository : IEquipmentQueryRepository
    {
        ...
        {
            IQueryable<EquipmentQuery> query = _inf370ContextDB.EquipmentQuery.Include(x => x.OnboarderID == id).Include(x=>x.Equipment).Include(x=>x.Onboarder).ThenInclude(x=>x.Employee);
